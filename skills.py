...
def common_items(list1, list2):
    ...
    for item in list1_set:
        common_items = common_items + ([item] * list2.count(item))
    
    for item in list2_set:
        common_items = common_items + ([item] * (list1.count(item)-1))
...
def unique_common_items(list1, list2):
    ...
    unique_common_items = list(list1_set & list2_set)
...
def sum_zero(list1):
    ...
    for i in range(len(list1)):
        for k in range(len(list1)):
            if k != i:
                key = (list1[i],list1[k])
                dictionary[key]= list1[i] + list1[k]
    ...
    for i in range(len(pairs)):
        pairs[i].reverse()
        if pairs[i] not in pairs[(i+1):]:
            nodup_pairs.append(pairs[i])
...
def word_length(words):
    ...
    for length in sorted(lens):
        len_list.append((length, dictionary[length]))
...
def pirate_talk(phrase):
    ...
    dictionary = {"sir":"matey","hotel":"fleabag inn","student":"swabbie","boy":"matey","madam":"proud beauty","professor":"foul blaggart","restaurant":"galley","your":"yer",
                  "excuse":"arr","students":"swabbies","are":"be","lawyer":"foul blaggart","the":"the'","restroom":"head","my":"me","hello":"avast","is":"be","man":"matey"}
    ...
    for word in sentence:
        if word in dictionary:
            new_sentence_list.append(dictionary[word])
        else:
            new_sentence_list.append(word)
    
    new_sentence_string = " ".join(new_sentence_list)
...
def adv_word_length_sorted_words(words):
    ...
    for length in sorted(lens):
        len_list.append((length, sorted(dictionary[length])))
